...
def EBE_get_headers_and_footers(content):
    ...
    for i in range(len(content)):
        text = content[i][0][0]
        bbox2 = content[i][1][0][1]
        if bbox2 < 50: # checks if the text is at the top and is in bold
...
#        new_content = content
#    else:
#        print("UIwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwIII")
#        check_if_2_column_in_page = False # this is to deal with pages with both tables and two column text
#        for i, j in enumerate(content):
#            text = content[i][0][0]
#            bbox0 = content[i][1][0][0]
...
#            if bbox0 > 280:
#                if '\uf0b7' in text:
#                    check_if_2_column_in_page = True
#        if check_if_2_column_in_page:
#            
#            new_content = new_c
#        else:
...
def check_sort_double_column_text(content, doc_dict):
    width = doc_dict['width'] # get the width of the page
    height =  width = doc_dict['height'] # get the height of the page
    new_c = content
    # This funtion deals with 2-columns page and sorting them accordingly
    new_content = []
    dominant_text_type = EBE_get_dominant_text_type(content) # check for page content type ie. table otr Text
    check_double_column = False
    for i in range(len(content)):
        if len(content[i][0][0]) == 2 and content[i][0][0][0] != '\uf0b7' and len(content[i][0][0]) < 5 and len(content[i][0][0]) > 1:
            check_double_column = True
        else:
            check_double_column = False
    if dominant_text_type == "text" and not check_double_column:
        #        check if there is a 2-column format in a text page
        double_column = False    
        for i, j in enumerate(content):
            bbox0 = content[i][1][0][0]
        ...
        new_check = []
        for i in range(len(content)):
            if len(content[i][0][0])  > 2:
                new_check.append(content[i][0][0])
        if len(new_check) > 4:
            check_table = True
        if double_column and not check_table:
            for i in range(len(content)):
                bbox_0 = content[i][1][0][0]
                if bbox_0 < 190:
                    new_content1.append(content[i])
...
def EBE_merge_bbox1_misalignment(content):
    """
    By Bruno
    This function merges elements the was splited but should be on same line.
    """
    ...
    for i, j in enumerate(content):
        ...
        calc = int(curr_bbox1) - int(prev_bbox2) # subtract the first digit before the decimal point
#        print('calc =  ',calc)
        check = False
        ...
        if int(curr_bbox1) != int(prev_bbox2) and calc > 5 or len(content) < 5:  # check if we had bbox that look alike in integer ie. the first digit before the decimal point. and as well, to deal with the case where the len of content is every small to check for previous content. usually page 1(s). 
...
def split_misaligned_line(content):
    """
    By Bruno
    This function seperates elments that merge together as one line, but should not be.
    """
    new_content = []
    for i, j in enumerate(content): # loop through the content
        curr_content = content[i]
        curr_content_bbox = content[i][1] # instantiate the bboxes
        
        f_indx = [] #get the index of where prev_bbox1 - curr_bbox1 is greater than 5
        g_indx = [] #get the index of where curr_bbox1 - prev_bbox1 is greater than 5
        try:
            for ii in range(len(curr_content_bbox)): # looping through the bbox1 to see if the previous minus current is greater 5. if so we seperate them to different line.
                nxt_bbox1_indx = curr_content_bbox[ii+1]
                curr_bbox1 = curr_content_bbox[ii][1]
                prev_bbox1 = curr_content_bbox[ii+1][1]
                if prev_bbox1 - curr_bbox1 > 5:
                    f_indx.append(curr_content_bbox.index(nxt_bbox1_indx))
                elif curr_bbox1 - prev_bbox1 > 5:
                    g_indx.append(curr_content_bbox.index(nxt_bbox1_indx))
...
#            print(e)
        ...
        if not f_indx or g_indx:
            new_content.append(curr_content)
        elif f_indx: # splitting the content
            curr_text = curr_content[0][0][:f_indx[0]]# slice the text A from B
            prev_text = curr_content[0][0][f_indx[0]:]
            
            curr_font = curr_content[0][1][:f_indx[0]]# slice the font A from B
            prev_font = curr_content[0][1][f_indx[0]:]
            
            curr_font_size = curr_content[0][2][:f_indx[0]]# slice the font size A from B
            prev_font_size = curr_content[0][2][f_indx[0]:]
            
            curr_bbox = curr_content[1][:f_indx[0]]# slice the bbox A from B
            prev_bbox = curr_content[1][f_indx[0]:]
            
            new_content1 = [[curr_text, curr_font, curr_font_size], curr_bbox]
        
            new_content2 = [[prev_text, prev_font, prev_font_size], prev_bbox]
            
            new_content.append(new_content1)
            new_content.append(new_content2)
            
        elif g_indx:
            curr_text = curr_content[0][0][:g_indx[0]]
            prev_text = curr_content[0][0][g_indx[0]:]
            
            curr_font = curr_content[0][1][:g_indx[0]]
            prev_font = curr_content[0][1][g_indx[0]:]
            
            curr_font_size = curr_content[0][2][:g_indx[0]]
            prev_font_size = curr_content[0][2][g_indx[0]:]
            
            curr_bbox = curr_content[1][:g_indx[0]]
            prev_bbox = curr_content[1][g_indx[0]:]
