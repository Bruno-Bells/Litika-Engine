...
def EBE_get_unique_chunks_v_1_0_3(content, threshold):
    ...
    max_gap, gap_sub = EBE_get_line_gaps(content)
...
def _counts(data): # code the count most frequent element in a list
...
def EBE_get_line_gaps(content):
    """
    This function gets the gap between the bottom of a line and the top of the next line
    
    """
    gap_list = [] # gap between the bottom of a line and the top of the next line
    for i, j in enumerate(content):
        try:
            bbox_1 = content[i+1][1][0][1]
            bbox_3 = content[i][1][0][3]
            gap = bbox_1 - bbox_3
            gap_list.append(gap)
        except:
            pass
    ...
    try:
        gap_threshold = mode(gap_list) + 6
        gap = mode(gap_list)
        gap_list.sort()
        max_gap = gap_threshold
        gap_sub = mode(gap_list) - 1

    except:
        gap_threshold = moder(gap_list) + 6
        max_gap = gap_threshold
        gap_sub = moder(gap_list) - 1
        
    return max_gap, gap_sub

#def EBE_get_unique_chunks_v_1_0_4(content):
##    last_element = content[i-1]
#    
#    line_chunk_label_list = []
#    
#    # sets initial value of the chunk to 1, then increases as new chunks are detected based
